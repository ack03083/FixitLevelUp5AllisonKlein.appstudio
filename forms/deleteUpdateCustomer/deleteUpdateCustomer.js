deleteUpdate.onclick=function(){
    ...
    if (req1.status == 200) {
       ...
       if (results.length == 0)
           txtResult.value("There are no company names.")
       else {        
           let message = ""
           for (i = 0; i <= results.length - 1; i++){
               message = message + results[i][0] + "\n"
               drpdwnCompany.addItem(results[i][0])
           }
       } 
   }else{
  ...
  hb2.clear()
  hb2.addItem("See Customers")
  hb2.addItem("Edit Customers")
  hb2.addItem("Delete Customers")
  hb2.addItem("Add Customers")
}


drpdwnCompany.onclick=function(s){
  if (typeof(s) == "object"){ 
    return
  } else {
    drpdwnCompany.value = s 
  }
}
...
if (rdoChoice.value ==0){
  ...
  let Name = drpdwnCompany.value
    ...
    if (found == false) 
      ...
      if (req4.status == 200) { 
        if (req4.responseText == 500){ 
            ...
            NSB.MsgBox("There was an error deleting " + Name + " from the database.")
        }
      } else {
   ...
   let oldName = drpdwnCompany.selection
